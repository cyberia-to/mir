...
pub fn on_enter_graph(
    ...
    let (w, h) = windows
        ...
        .map(|win| {
            render_size(
                win.resolution.physical_width() as f32,
    ...
    let epoch_arc: Arc<RwLock<Option<crate::epoch::EpochState>>> = Arc::new(RwLock::new(None));
...
pub fn sync_visible_entities(mut gpu: ResMut<GpuBuffers>, cam: Res<GraphCamera>) {
    ...
    if gpu.cached_vp == Some(camera.view_proj) {
        return;
    }
    ...
    } else if let (Some(cull), Some(pb), Some(rb)) = (&gpu.cull, &gpu.pos_buf, &gpu.rad_buf) {
    ...
    let vis_set: std::collections::HashSet<u32> = visible.iter().map(|&(idx, _)| idx).collect();
...
pub fn dispatch_tiers(
    ...
    let Some(cmd) = gpu.sync_queue.as_ref().and_then(|q| q.commands().ok()) else {
        return;
    };
    if let (Some(paint), Some(f8)) = (&gpu.paint, &gpu.frame_u8) {
        trace_step("paint");
        let t0 = std::time::Instant::now();
        let drawn = paint.draw(
            &gpu.sorted,
            &gpu.visible,
            &gpu.pos_cpu,
            &gpu.rad_cpu,
...
#[cfg(not(target_vendor = "apple"))]
pub fn publish_frame(gpu: Res<GpuBuffers>, handoff: Res<crate::bevy::blit::FrameHandoff>) {
    if gpu.settle > 0 {
        return;
    }
    let (Some(buf), Some(handle)) = (&gpu.frame_u8, &gpu.output_image) else {
        return;
    };
    let [w, h] = gpu.viewport;
...
pub fn track_frame_size(
    ...
    let Some(handle) = gpu.output_image.clone() else {
        return;
    };
...
pub fn composite(gpu: Res<GpuBuffers>, mut images: ResMut<Assets<Image>>) {
    let (Some(pixels), Some(handle)) = (&gpu.last_pixels, &gpu.output_image) else {
        return;
    };
    let Some(image) = images.get_mut(handle) else {
        return;
    ...
    if data.len() != expected || pixels.len() < expected {
        return;
    }
...
fn dump_frame_once(rgba: &[u8], w: u32, h: u32) {
    ...
    let Ok(path) = std::env::var("MIR_DUMP_FRAME") else {
        return;
    };
...
/// through two systems.
pub static COMPOSITE_MS: std::sync::atomic::AtomicU32 = std::sync::atomic::AtomicU32::new(0);
...
pub fn warp_to_system(
    ...
    let Some(idx) = target.particle_idx.take() else {
        return;
    };
